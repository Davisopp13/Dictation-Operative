...
void test('Windows downloads require sign-in and serve only reviewed packages', async (t) => {
  ...
  try {
    ...
    const s: Services = {
      ...
      images: {
        ...
        get: async (key) => {
          reads++;
          const release = ['x64', 'arm64'].map(windowsRelease).find(r => r?.key === key);
          assert.ok(release, 'only an allowlisted release key may be read');
          if (missing) return null;
          lastBody = new ReadableStream<Uint8Array>({ start(controller) { controller.enqueue(new TextEncoder().encode('synthetic ZIP stream')); }, cancel() { canceled++; } });
          return { body: lastBody, size: mismatch ? 3 : release.size };
        },
      },
    };
    const call = (path: string, method = 'GET', services = s) => handleAPI(new Request(`https://do.test/api/windows-downloads/${path}`, { method, headers: { Origin: 'https://do.test' } }), services);
    await t.test('unsigned requests cannot read the bucket', async () => {
      assert.equal((await call('x64', 'GET', { ...s, owner: '' })).status, 401);
    ...
    await t.test('unknown architectures, object paths and writes are rejected', async () => {
      for (const path of ['__proto__', 'constructor', 'x86', 'x64/extra', '%2e%2e%2fclipboard%2fprivate.png'])
        assert.equal((await call(path)).status, 404);
      assert.equal((await call('x64', 'POST')).status, 405);
      assert.equal(reads, 0);
    });
    await t.test('each architecture streams the expected attachment without buffering', async () => {
      for (const arch of ['x64', 'arm64']) {
        const response = await call(arch);
        const release = windowsRelease(arch)!;
    ...
    await t.test('HEAD supplies metadata and cancels the unused stream', async () => {
      ...
      const response = await call('x64', 'HEAD');
      assert.equal(response.status, 200);
      assert.equal(response.body, null);
      assert.equal(canceled, before + 1);
    });
    await t.test('missing storage and absent packages fail clearly', async () => {
      assert.equal((await call('x64', 'GET', { ...s, images: undefined })).status, 503);
      missing = true;
      assert.equal((await call('x64')).status, 503);
      missing = false;
    });
